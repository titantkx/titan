...
function checkTestEnv() {
  ...
  if (!argv.network) {
    ...
    if (argv.network !== 'titan' && argv.network !== 'ganache') {
      panic('network is invalid. Must be ganache or titan');
    }
    else {
...
function performTestSuite({ testName, network }) {
  const cmd = network === 'ganache' ? 'test-ganache' : 'test-titan';
...
function setupNetwork({ runConfig, timeout }) {
  if (runConfig.network !== 'titan') {
    // no need to start ganache. Truffle will start it
    return;
  }

  // Spawn the titan process

  const spawnPromise = new Promise((resolve, reject) => {
    const titandProc = spawn('./init-test-node.sh', {
      cwd: __dirname,
      stdio: ['ignore', runConfig.verboseLog ? 'pipe' : 'ignore', 'pipe'],
    });

    logger.info(`Starting titand process... timeout: ${timeout}ms`);
    if (runConfig.verboseLog) {
      titandProc.stdout.pipe(process.stdout);
    }
    titandProc.stderr.on('data', d => {
      const oLine = d.toString();
      if (runConfig.verboseLog) {
        process.stdout.write(oLine);
      }
      if (oLine.indexOf('Starting JSON-RPC server') !== -1) {
        logger.info('titand started');
        resolve(titandProc);
      }
    });
  });
  ...
  const timeoutPromise = new Promise((resolve, reject) => {
    setTimeout(() => reject(new Error('Start titand timeout!')), timeout);
  });
