...
func (s *KeeperTestSuite) TestBurnDenomMsg() {
	...
	// mint 10 default token for addr0
...
func (s *KeeperTestSuite) TestCreateDenomMsg() {
	...
	for _, tc := range []struct {
		...
		s.Run(fmt.Sprintf("Case %s", tc.desc), func() {
			...
			_, err := s.msgServer.CreateDenom(ctx, types.NewMsgCreateDenom(addr0.String(), tc.subdenom))
			if tc.valid {
				s.Require().NoError(err)
			} else {
				s.Require().Error(err)
...
func (s *KeeperTestSuite) TestChangeAdminDenomMsg() {
	...
	for _, tc := range []struct {
		...
		s.Run(fmt.Sprintf("Case %s", tc.desc), func() {
			...
			res, err := s.msgServer.CreateDenom(ctx, types.NewMsgCreateDenom(addr0.String(), "bitcoin"))
			s.Require().NoError(err)
			testDenom := res.GetNewTokenDenom()
			_, err = s.msgServer.Mint(ctx, types.NewMsgMint(addr0.String(), sdk.NewInt64Coin(testDenom, 10)))
			s.Require().NoError(err)
