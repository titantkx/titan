...
func TestDeposits(t *testing.T) {
	...
	TestAddrs := simtestutil.AddTestAddrsIncremental(bankKeeper, ctx, genAddr, 2, sdk.NewInt(10_000_000), utils.BaseDenom)
	...
	fourStake := sdk.NewCoins(sdk.NewCoin(utils.BaseDenom, stakingKeeper.TokensFromConsensusPower(ctx, 4)))
	fiveStake := sdk.NewCoins(sdk.NewCoin(utils.BaseDenom, stakingKeeper.TokensFromConsensusPower(ctx, 5)))
	...
	require.NoError(t, err)
	govKeeper.DeleteAndContributeDeposits(ctx, proposalID)
