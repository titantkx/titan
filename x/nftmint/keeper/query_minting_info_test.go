...
func TestMintingInfoQueryPaginated(t *testing.T) {
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(msgs); i += step {
			resp, err := keeper.MintingInfos(wctx, request(nil, uint64(i), uint64(step), false)) //nolint:gosec
			require.NoError(t, err)
	...
	t.Run("ByKey", func(t *testing.T) {
		...
		var next []byte
		for i := 0; i < len(msgs); i += step {
			resp, err := keeper.MintingInfos(wctx, request(next, 0, uint64(step), false)) //nolint:gosec
			require.NoError(t, err)
			require.LessOrEqual(t, len(resp.MintingInfo), step)
			require.Subset(t,
				nullify.Fill(msgs),
